...
Function's docker builds only install the tools required for configured `codegen` build. The final docker image is Alpine based and includes only `watchdog`, and the executable for `fprocess`.
