...
Default code generator is Chez scheme.

For all code generators, library configuration is the same as in <<fn-libs>>.

When choosing a code generator, only the build tools that are required for that code generator are installed into the final docker image.

=== Node code generator ===

Add a `package.json` file to the root directory of your Function containing whichever dependencies you need and a `pack.toml` file containing:
[source]
----
[idris2]
codegen = "node"
...
* Support other Idris 2 language back ends: https://github.com/mmhelloworld/idris-jvm[JVM], https://github.com/madman-bob/idris2-python[Python]
* Run Function as non-root user
* Use official idris2-pack image to provide `idris2` and `pack`
* Create separate templates for different backends
* add Node Function tests
